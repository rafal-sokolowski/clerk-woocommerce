	...
	public function add_powerstep_files() {
		...
		try {
			...
			if ( is_page( $options['powerstep_page'] ) ) {
				wp_enqueue_style( 'clerk_powerstep_css', plugins_url( '../assets/css/powerstep.css', __FILE__ ), array(), 'TEST_STUBBE' );
			}

			wp_enqueue_script( 'clerk_powerstep_js', plugins_url( '../assets/js/powerstep.js', __FILE__ ), array( 'jquery' ), 'TEST_STUBBE', true );
			wp_localize_script(
				'clerk_powerstep_js',
				'variables',
				array(
